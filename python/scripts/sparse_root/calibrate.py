...
Usage
-----
  python VoxelTree/scripts/sparse_root/calibrate.py \\
...
def create_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Path to sparse_root_pairs.npz training data",
    )
    parser.add_argument(
        "--model",
        type=Path,
        help="Path to sparse_root.onnx model (required unless --config is given)",
    )
    parser.add_argument(
        "--config",
        type=Path,
        help="Path to sparse_root_config.json (will auto-locate model if --model not given)",
    )
    parser.add_argument(
        "--samples",
        type=int,
...
def load_training_data(npz_path: Path, num_samples: int, seed: int = 42) -> np.ndarray:
    ...
    Args:
        npz_path: Path to sparse_root_pairs.npz
...
def find_model_path(config_path: Path) -> Path:
    ...
    onnx_path = config_dir / "sparse_root.onnx"
    if onnx_path.exists():
        return onnx_path

    # Try without directory suffix
    stem = config_dir.stem
    if stem.startswith("sparse_root_"):
        return config_dir / "sparse_root.onnx"

    raise FileNotFoundError(
        f"Cannot locate sparse_root.onnx near {config_path}; " f"please specify --model explicitly"
    )
