...
"""
Distillation module for density NNs, with progress callback support for GUI integration.
"""
import copy
import math
import json
...
from VoxelTree.notebooks.experimental.density_nn_shootout import CANDIDATES, count_parameters
...
def find_repo_root() -> Path:
    for candidate in [Path.cwd(), *Path.cwd().parents]:
        if (candidate / 'noise_training_data' / 'phase_1a_data.npz').exists():
            return candidate
    raise FileNotFoundError('Could not find repo root containing noise_training_data/phase_1a_data.npz')
...
def distill_student(teacher_name, student_name, epochs, alpha, lr, device, progress_callback=None):
    ...
    for epoch in range(epochs):
        ...
        if progress_callback:
            progress_callback(epoch + 1, epochs, row)
