        ...
        for i, p in enumerate(self.points):
            if p['derivative'] is None:
                if len(self.points) == 1:
                    ...
                    p['derivative'] = (self.points[i+1]['value'] - self.points[i-1]['value']) / (
                        self.points[i+1]['location'] - self.points[i-1]['location']
                    )
        
        spline_points = [
            CubicSplinePoint(p['location'], p['value'], p['derivative'])
...
AMPLIFIED_OFFSET = lambda offset: offset if offset < 0 else offset * 2.0
AMPLIFIED_FACTOR = lambda factor: 1.25 - 6.25 / (factor + 5.0)
AMPLIFIED_JAGGEDNESS = lambda jaggedness: jaggedness * 2.0
...
def build_ridge_jaggedness_spline(weirdness_spline_func, ridges_val, jaggedness_factor_at_peak_ridge, jaggedness_factor_at_high_ridge, jaggedness_transformer):
...
def build_erosion_jaggedness_spline(erosion, weirdness, ridges, 
...
def main():
    ...
    # Export as ONNX
    output_dir = Path(__file__).parent.parent / "src" / "main" / "resources" / "assets" / "lodiffusion" / "models"
