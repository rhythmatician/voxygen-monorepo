...
def stack_and_save_sparse_octree(pairs: list[dict[str, Any]], cache_path: Path) -> int:
...
def build(
    ...
    *,
    ...
    clean: bool = False,
    model_type: str = "all",
    sparse_octree: bool = False,
    sparse_octree_output: Path | None = None,
) -> tuple[int, int]:
    ...
    Args:
        ...
        sparse_octree: If True, generate an additional sparse-root cache.
        sparse_octree_output: Output path for sparse-root cache (default:
            ``<output_dir>/sparse_octree_pairs.npz``).
    ...
    sparse_octree_cache = sparse_octree_output or (output_dir / "sparse_octree_pairs.npz")
    ...
    if clean:
        ...
        if sparse_octree and sparse_octree_cache.exists():
            print(f"  Removing stale cache: {sparse_octree_cache.name}")
            sparse_octree_cache.unlink()
...
""")
        ...
        sparse_octree_count = stack_and_save_sparse_octree(sparse_pairs, sparse_octree_cache)
        print(f"  Sparse-root pairs: {sparse_octree_count:,} (from {len(all_pairs):,} sections)")
        print()
    ...
    marker.write_text(
        ...
        f"val_pairs: {n_val}\n"
        f"sparse_octree_pairs: {sparse_octree_count}\n"
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    build(
        ...
        sparse_octree=args.sparse_octree,
        sparse_octree_output=args.sparse_octree_output,
    )
