...
from voxel_tree.tasks.density.density_nn_candidates import CANDIDATES
...
def find_repo_root() -> Path:
    for candidate in [Path.cwd(), *Path.cwd().parents]:
        if (candidate / "noise_training_data" / "terrain_shaper_density_data.npz").exists():
            return candidate
    raise FileNotFoundError(
        "Could not find repo root containing noise_training_data/terrain_shaper_density_data.npz"
    )


REPO_ROOT = find_repo_root()
DATA_PATH = REPO_ROOT / "noise_training_data" / "terrain_shaper_density_data.npz"
ARTIFACT_DIR = (
