...
Loads the trained Stage 1 model (`terrain_shaper_model/terrain_shaper_mlp.pt`), re-exports:
  - terrain_shaper_mlp_weights.bin  — flat float32 SSBO blob
  - terrain_shaper_norm_mean.bin    — float32[12] input normalisation mean
  - terrain_shaper_norm_std.bin     — float32[12] input normalisation std
...
Usage:
  python extract_terrain_shaper_weights.py [--model-dir PATH] [--out-dir PATH]

Layout of terrain_shaper_mlp_weights.bin:
...
# ---------------------------------------------------------------------------
# Mirror the model architecture from train_terrain_shaper_density.py
# ---------------------------------------------------------------------------
...
def load_checkpoint(model_dir: Path) -> dict:
    ckpt_path = model_dir / "terrain_shaper_mlp.pt"
    if not ckpt_path.exists():
        # Fall back to best checkpoint
        ckpt_path = model_dir / "terrain_shaper_mlp_best.pt"
    if not ckpt_path.exists():
        print(
            f"[ERROR] No checkpoint found in {model_dir}  —  train first with "
            f"train_terrain_shaper_density.py",
            file=sys.stderr,
        )
        sys.exit(1)
...
def export_flat_weights(model: DensityNN, out_path: Path) -> None:
...
def export_norm_stats(ckpt: dict, out_dir: Path) -> None:
    ...
    mean_path = out_dir / "terrain_shaper_norm_mean.bin"
    std_path = out_dir / "terrain_shaper_norm_std.bin"
...
def verify_model(model: DensityNN, ckpt: dict) -> None:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        "--model-dir",
        default="terrain_shaper_model",
        help="Directory containing terrain_shaper_mlp.pt  (default: terrain_shaper_model)",
    )
    p.add_argument(
...
def main(argv: list[str] | None = None) -> None:
    ...
    weights_bin = out_dir / "terrain_shaper_mlp_weights.bin"
    ...
    if not args.no_deploy:
        ...
        to_copy = [
            (out_dir / "terrain_shaper_mlp_weights.bin", "terrain_shaper_mlp_weights.bin"),
            (out_dir / "terrain_shaper_norm_mean.bin", "terrain_shaper_norm_mean.bin"),
            (out_dir / "terrain_shaper_norm_std.bin", "terrain_shaper_norm_std.bin"),
        ]
        # Also copy ONNX if present
        onnx_src = model_dir / "terrain_shaper_mlp.onnx"
        if onnx_src.exists():
            to_copy.append((onnx_src, "terrain_shaper_mlp.onnx"))
