...
def cmd_dumpnoise(cfg: PipelineConfig) -> None:
    ...
    with RconClient(cfg.host, cfg.port, cfg.password) as rcon:
        ...
        # the 4 concurrent TerrainShaperDumper-Worker threads start making 8 MB
        ...
        while time.time() - start < timeout:
            ...
            # Heuristic: ~200 chunks/sec is typical for TerrainShaper
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_dn = sub.add_parser(
        "dumpnoise",
        parents=[shared, pregen_args],
        help="Consolidate TerrainShaper + SparseOctree noise dumps via RCON (both formats needed for training)",
    )
    p_dn.add_argument(
        "--timeout",
        type=int,
