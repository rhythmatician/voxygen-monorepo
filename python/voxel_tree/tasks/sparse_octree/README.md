...
- **Training**: `VoxelTree.scripts.sparse_octree.sparse_octree_train` → Training loop, data handling
- **Distillation**: `VoxelTree.scripts.sparse_octree.sparse_octree_distill` → Knowledge distillation from octree
- **Utilities**: `VoxelTree.scripts.sparse_octree_targets` → Ground truth generation

### Training Scripts (this folder)
- `train.py` — Training entry point launched via GUI (reads from `VoxelTree.scripts.sparse_octree.*`)
...
- `diagnose_sparse_octree_data.py` — Data validation
- `list_biomes_in_training_set.py` — Dataset inspection
- `sparse_octree_exploration.ipynb` — Architecture experiments
- `sparse_octree_runtime_tradeoffs.ipynb` — Speed vs. accuracy analysis
...
2. **Build pairs** → `build_pairs_sparse_octree` (creates input data)
3. **Train** → `train_sparse_octree` (train.py)
4. **Distill** → `distill_sparse_octree` (distill.py) — optional knowledge distillation
