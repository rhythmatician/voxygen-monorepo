...
Usage
-----
  python VoxelTree/scripts/sparse_octree/diagnose.py \\
...
def create_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Path to sparse_octree_pairs.npz",
    )
    parser.add_argument(
        "--samples",
        type=int,
...
def analyze_samples(npz_path: Path, num_samples: int, verbose: bool = False) -> None:
    ...
    print(f"{'='*70}")
    print(
        f"All-air samples:       {air_only_samples:5d} ({air_only_samples/num_samples*100:5.1f}%)"
    )
    print(
        f"Solid-only samples:    {solid_only_samples:5d} ({solid_only_samples/num_samples*100:5.1f}%)"
    )
    ...
    print(
        f"Final density range (across means):   [{np.min(final_density_means):6.3f}, {np.max(final_density_means):6.3f}]"
    )
