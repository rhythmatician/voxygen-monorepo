"""Build sparse-root training pairs from sparse_root noise dumps + Voxy L4 sections.
...
Usage
-----
  python scripts/build_sparse_root_pairs.py \\
      --dumps  VoxelTree/tools/fabric-server/runtime/sparse_root_dumps \\
      --voxy   VoxelTree/data/voxy_octree \\
      --output noise_training_data/sparse_root_pairs.npz
...
NOISE_FIELDS = [
    ...
    "y",
    "entrances",
    "pillars",  # vanilla: overworld/caves/pillars (was "cheese_caves" in stage1)
    "spaghetti_2d",
    "spaghetti_roughness",  # vanilla: spaghetti_roughness_function (was "roughness" in stage1)
    "noodle",
    "base_3d_noise",
    "final_density",
]
...
def build_pairs(
    ...
    if not dump_files:
        ...
        sys.exit(1)
    print(f"  Found {len(dump_files):,} sparse_root section dump files")
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--dumps",
        type=Path,
        default=Path("VoxelTree/tools/fabric-server/runtime/sparse_root_dumps"),
        metavar="DIR",
        help="Directory containing section_*.json sparse_root dumps",
    )
    parser.add_argument(
        "--voxy",
        type=Path,
