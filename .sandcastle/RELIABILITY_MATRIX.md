...
| 5 | Stale Sandcastle `dist` vs source identity (`eeae29e`) | #149 Doctor provenance | Runtime Sandcastle code has provable source identity | Doctor checks `dist/index.js` contains `Agent alive`, `No observable output`, and mtime/hash cached | `doctor.test: runtime artifact provenance` |
...
| No `rhythmatician/sandcastle` library change this round | `sandcastle` fork | Deadman `1800s` already `eeae29e`; worktree lifecycle already principled; next library change only if new worktree-timeout evidence appears — deliberate pause |
