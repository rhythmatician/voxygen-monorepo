...
describe("Parent map pointer extraction", () => {
  it("extracts Part of #N for parent map pointer", async () => {
    const { default: fs } = await import("node:fs");
    const main = fs.readFileSync(".sandcastle/main.mts", "utf8");
    // Verify main contains Part of # handling
    expect(main).toContain("Part of #");
    expect(main).toContain("extractParentMapId");
    expect(main).toContain("addParentMapPointer");
  });
});

describe("Research worker exact body transport and branch invariant", () => {
  it("main.mts researcher receives exact host-selected issue title/body and dedicated managed worktree/sandbox", async () => {
    const fs = await import("node:fs");
    const main = fs.readFileSync(".sandcastle/main.mts", "utf8");
    // Exact body transport: researcher prompt uses ISSUE_BODY from host-fetched src.body
    expect(main).toContain("ISSUE_BODY: exactBody");
    expect(main).toContain("ISSUE_TITLE: exactTitle");
    expect(main).toContain("researchSourceMap");
    expect(main).toContain("exactBody = src.body");
    // Dedicated managed worktree/sandbox per ticket from frozen factory base
    expect(main).toContain("createSandbox");
    expect(main).toContain("branch: issue.branch");
    expect(main).toContain("baseBranch: factoryBaseSha");
    // Caller branch/HEAD/status remain unchanged — same invariant as implement
    expect(main).toContain("callerStatusBefore");
    expect(main).toContain("verifyCallerUnchanged");
    expect(main).toContain("getResearchEnvironment");
  });

  it("research success requires no commits and never calls implementer review/merger/push/PR/auto-merge", async () => {
    const fs = await import("node:fs");
    const main = fs.readFileSync(".sandcastle/main.mts", "utf8");
    // Research path should not contain reviewVerdict or merger for research
    // Isolate research block: ensure no reviewVerdictSchema inside research map
    const researchBlock = main.slice(main.indexOf("Research Phase"), main.indexOf("Phase 2: Execute + Review"));
    expect(researchBlock).not.toContain("reviewVerdictSchema");
    expect(researchBlock).not.toContain("runReviewerPass");
    expect(researchBlock).not.toContain("merger");
    expect(researchBlock).not.toContain("publishBatchBranch");
    expect(researchBlock).not.toContain("mayAutonomouslyMerge");
    // Research block should explicitly note ignoring commits
    expect(researchBlock).toContain("research should not commit");
  });
});

describe("FACTORY_ERROR semantics for research", () => {
  it("invalid structured output maps to FACTORY_ERROR (release, no conclusion, stop)", async () => {
    const fs = await import("node:fs");
    const main = fs.readFileSync(".sandcastle/main.mts", "utf8");
    // Invalid output throws and is treated as rejected => markResearchFactoryError
    expect(main).toContain("research structured output invalid");
    expect(main).toContain("markResearchFactoryError");
    expect(main).toContain("researchHadFactoryError = true");
    // FACTORY_ERROR release: remove claim, leave open retryable (markFactoryError does this)
    expect(main).toContain("markFactoryError");
    // Publication failure also FACTORY_ERROR
    expect(main).toContain("publication failed for research");
    // Stop unsafe outer progression
    expect(main).toContain("Research FACTORY_ERROR stops outer loop");
  });

  it("sandbox/provider failure is FACTORY_ERROR not blocked", async () => {
    const fs = await import("node:fs");
    const main = fs.readFileSync(".sandcastle/main.mts", "utf8");
    // Sandbox creation retry then throw => rejected => FACTORY_ERROR, not agent:blocked
    expect(main).toContain("research sandbox create failed");
    // Verify that markResearchFactoryError delegates to markFactoryError which removes in-progress+assignee, not adding blocked
    expect(main).toContain("markResearchFactoryError");
    // Ensure siblings not erased: siblings' successes preserved via continue loop
    expect(main).toContain("siblings' successes must survive");
  });
});

describe("Concurrency regression: three research workers overlap", () => {
  it("proves all three isolated researchers are active concurrently before any is released", async () => {
    let active = 0;
    let maxActive = 0;
    let started = 0;
    ...
    async function fakeResearcher(id: number): Promise<number> {
      active++;
      maxActive = Math.max(maxActive, active);
      started++;
      if (started === 3) {
        // All three started — signal barrier after a tick
        setTimeout(() => barrierResolve(), 5);
      }
      await barrier;
      // Small delay to ensure overlap window measurable
      await new Promise((r) => setTimeout(r, 10));
      active--;
      return id;
    }

    const settled = await Promise.allSettled([fakeResearcher(1), fakeResearcher(2), fakeResearcher(3)]);
    expect(settled.every((s) => s.status === "fulfilled")).toBe(true);
    expect(maxActive).toBe(3);
    // Prove not sequential: if sequential, maxActive would be 1
    expect(maxActive).not.toBe(1);
  });

  it("failure in one researcher must not erase valid results already produced by siblings", async () => {
    async function worker(id: string): Promise<{ id: string; result?: ResearchResult }> {
      if (id === "11") throw new Error("infrastructure failure");
      return { id, result: { summary: `result ${id}`, findings: [], recommendation: "ok", uncertainties: [], followUps: [] } };
    }
    const issues = [{ id: "10" }, { id: "11" }, { id: "12" }];
    const settled = await Promise.allSettled(issues.map((iss) => worker(iss.id)));
    // Simulate host-side publication: successes published even though one failed
    const successes: string[] = [];
    const failures: string[] = [];
    for (let i = 0; i < issues.length; i++) {
      const s = settled[i]!;
      if (s.status === "fulfilled") successes.push(issues[i]!.id);
      else failures.push(issues[i]!.id);
    }
    expect(successes).toEqual(["10", "12"]);
    expect(failures).toEqual(["11"]);
    // Siblings' successes preserved
