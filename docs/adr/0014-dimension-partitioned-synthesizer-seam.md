...
* Biome variation is `BiomeEligibility.isChorusBiome(blockX,blockZ)` chorus-local predicate injected per dimension for End, not a top-level BiomeSynthesizer nor a generic `FeatureEligibility`. Placed features remain independent responsibilities per Worldgen Partition v1.
* VoxelVolumeWriter.writeRegion remains the storage seam; Worldsection Synthesizer is not a separate concept.
* EndDimensionSynthesizer is the End implementation of DimensionSynthesizer; EndChorusSynthesizer remains a chorus-specific helper owned by EndDimensionSynthesizer, not the dimension seam itself. OverworldSynthesizer/NetherSynthesizer are siblings, not branches.
...
* GenerationSession.produceEndRefinementChild becomes delegation -> DimensionSynthesizers.forDimension(boundDimension).synthesize(...); EndL4DeterministicCandidate/ExactEndL1Candidate move behind the dimension implementation.
...
* Level-aware honest omission (L4/L3 chorus 0) is owned per dimension without a global BiomeSynthesizer.
