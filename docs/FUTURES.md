...
| Renderer-informed occlusion culling | candidate (not registered) | — | Future generation/refinement optimization using conservative renderer visibility evidence (for example Voxy traversal state, depth/Hi-Z, or another occlusion hierarchy) to soft-cull terrain hidden behind nearer geometry. The End slice only needs cheap frustum gating; any renderer→scheduler seam must fail open and must never make transient visibility into terrain truth. |
| Cross-dimension visibility and prefetch policy | candidate (not registered) | — | After the End slice, revisit movement-direction prefetch and visibility heuristics for Overworld/Nether, where mountains, valleys, caves, and the Nether ceiling change what “useful next refinement” means. Preserve scheduler inputs as replaceable geometric hints rather than baking End-specific assumptions into generation. |
...
| Multi-version Minecraft/Voxy support | registered future | #237 | Minecraft 1.20.4 is the permanent support floor; 1.20.3 and earlier are forever out of scope. Future version pairs should require bounded profile/adapter/oracle/model regeneration as needed, not per-version architecture forks. |
