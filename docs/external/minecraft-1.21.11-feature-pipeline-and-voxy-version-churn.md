...
2. **The feature content is overwhelmingly data/configuration and belongs in a normalized profile.** The datapack registry bodies (`OreFeatures` 11.1 KB, `VegetationFeatures` 35.9 KB, `TreeFeatures` 31.9 KB, `CaveFeatures` 18.5 KB, `VegetationPlacements` 37.8 KB, `OrePlacements` 17.6 KB, `TreePlacements` 16.6 KB, `OverworldBiomes` 49.6 KB) describe what to place, where, and how often. A new MC version is *expected* to change these bodies (new biomes, new features, new rarity curves) rather than the engine. This pattern is **partially verified over the sampled span** for feature registry/configuration content (§3.5); placement and biome registries remain expectation-only rather than semantically diffed. The most defensible cost-reduction is to express the bodies as **versioned data fixtures** behind a single execution engine, not to reimplement per-version.
...
| Voxy 0.2.11-alpha | jar at `tools/server-harness/runtime/mods/voxy-0.2.11-alpha.jar (pinned via .ci/voxy-artifact.json, reconstructed by tools/server-harness/scripts/install.sh)`; source `external/voxy` | jar SHA-256 `63d1747017041b659ef620f589006d079d3574e3124dbdb165f9998533a7920c`; source commit `337b919d` on `dev` branch (2026-08-10); jar metadata is `python/voxel_tree/voxy_format.py` mirror | Audited, version-pinned in `docs/reference/upstream/voxy-0.2.11-alpha-storage-and-lod-seams.md`. |
...
| Evidence axis | Verdict (1.21.11) | Evidence | cross-MC-version evidence |
|---|---|---|---|
| Semantic stability | High -- `finalDensity > 0.0` determines base solid occupancy; aquifer/fluid filling, ore veins, surface material assignment, and carvers are separate layered responsibilities. `SURFACE_DENSITY_THRESHOLD = 1.5625` belongs to the `slopedCheese` surface/underground branch and is not the solid/empty threshold | `NoiseRouterData.java:29,184`; `Aquifer.java:38,135`; `OreVeinifier.java:15`; `SurfaceSystem.java:77`; `carver/WorldCarver.java`; GLOSSARY.md `NoiseRouter` entry | none-local |
...
| Residual complexity | An exact `finalDensity` port can eliminate the learned residual for **base solid/empty occupancy only**; the aquifer, surface, ore-vein, and carver responsibilities grouped under S3-S6 retain their own residuals. If learned instead, residual = `finalDensity` per block (already done by the Voxygen `finalDensity>0` MLP pattern) | `python/voxy_models.py` | n/a |
...
The shape is **a stream-flatMap over a generic `getPositions(PlacementContext, RandomSource, BlockPos) → Stream<BlockPos>`** and a terminal `ConfiguredFeature.place(WorldGenLevel, ChunkGenerator, RandomSource, BlockPos)`. Every dimension's feature pipeline and every biome's placed-feature list runs through this exact shape. This claim is scoped to **placed-feature execution within biome decoration** -- structures share the `GenerationStep.Decoration` stage but are not `PlacedFeature` executions and are a separate responsibility. There is no dimension- or family-specific override of the *machinery*.
...
**Why this is "stable/shared":** the loop is a **single, generic, dimension-agnostic** operation: a chain of `PlacementModifier.getPositions` and one terminal `ConfiguredFeature.place`. It is used by **every dimension and every biome's placed-feature execution within biome decoration** -- the only specialization is the `List<PlacementModifier>` chain contents and the `ConfiguredFeature` impl. Structures share the decoration stage but are not `PlacedFeature` executions. If Voxygen were to port this loop, the cost is ~25 lines of code; the rest is data.
