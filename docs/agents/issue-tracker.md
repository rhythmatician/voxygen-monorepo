...
- **Research**: `agent:research` + `wayfinder:research` (open, unassigned, unblocked, unclaimed, `blocked_by=0`). `ready-for-agent` is triage only. `agent:research` without `wayfinder:research` or `agent:implement + agent:research` fails closed. Dispatched via Sandcastle parallel research profile: isolated worktree/sandbox per ticket from frozen `origin/main`, Muse researcher with host-fetched body, strict structured result (`summary, findings[{claim,evidence,source}], recommendation, uncertainties[], followUps[]`), host-side publication + required parent-map pointer (`Part of #N`) + close retaining `agent:research`. Research success does not require commits; research may produce commits on its dedicated branch for durable knowledge artifacts but those commits must not enter implementation review, merger, batch integration, push, PR creation, or auto-merge. Result and parent-pointer comments carry idempotency markers. Invalid output or infrastructure/provider/publication/parent-pointer/close failure is `FACTORY_ERROR` (release transient claim, retain `agent:research`, retryable, stop outer loop, no `agent:blocked`).
