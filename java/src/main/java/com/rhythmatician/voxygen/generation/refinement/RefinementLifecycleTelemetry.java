...
public final class RefinementLifecycleTelemetry {
    ...
    public synchronized void recordDequeued(int parentLevel) {
        if (tracked(parentLevel)) counts[parentLevel][DEQUEUED]++;
    }

    public synchronized void recordAttempt(int parentLevel, RefinementOutcome outcome) {
        if (!tracked(parentLevel) || outcome == null) return;
        counts[parentLevel][DEQUEUED]++;
        recordOutcome(parentLevel, outcome);
    }

    public synchronized void recordOutcome(int parentLevel, RefinementOutcome outcome) {
        if (!tracked(parentLevel) || outcome == null) return;
