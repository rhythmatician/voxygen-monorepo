 ...
 */
public final class VanillaOccupancyPyramid {
    ...
    public record Cell(int level, int x, int y, int z) {
        public Cell {
    ...
    public record Delta(Set<Cell> addedUrgent, Set<Cell> removedUrgent, Set<Cell> newlyMixedParents) {
        public Delta {
    ...
    public Delta observeVanillaOctant(int l0X, int l0Y, int l0Z, int octant) {
        if (octant < 0 || octant > 7) {
            throw new IllegalArgumentException("octant must be 0..7: " + octant);
        }
     ...
     */
    public Delta observeVanillaL0Octants(int x, int y, int z, int ownedOctantMask) {
    ...
    public Occupancy classify(Cell cell) {
    ...
    public Relation relation(Cell cell) {
    ...
    public int missingChildOctants(Cell parent) {
        if (parent.level() == 0) {
    ...
    public Set<Cell> urgentBoundary() {
