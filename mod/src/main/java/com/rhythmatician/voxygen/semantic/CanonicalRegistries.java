...
public final class CanonicalRegistries {
    ...
    /**
     * Canonical block identifier for a canonical ID (0..1103). Single source of truth for the 1104 superset,
     * verified against {@code config/voxy_vocab.json} (SHA 9b034f2f...). Backed by the same array previously in
     * {@code CanonicalVoxyMaps}.
     */
    public static String canonicalName(int canonicalId) {
        if (canonicalId < 0 || canonicalId >= CANONICAL_BLOCK_NAMES.length) {
            throw new IllegalArgumentException("invalid canonicalId " + canonicalId);
        }
        return CANONICAL_BLOCK_NAMES[canonicalId];
    }

    private static final String[] CANONICAL_BLOCK_NAMES = {
        "minecraft:air",
